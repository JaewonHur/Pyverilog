...
class VerilogParser(PLYParser):
    ...
    """ JWHUR added """
    def p_integer_substitution(self, p):
    ...
    def p_taskcall(self, p):
        'taskcall : identifier LPAREN task_args RPAREN SEMICOLON'
        p[0] = FunctionCall(p[1], p[3], lineno=p.lineno(1))
        p.set_lineno(0, p.lineno(1))

    def p_task_args(self, p):
        'task_args : task_args COMMA expression'
        p[0] = p[1] + (p[3],)
        p.set_lineno(0, p.lineno(1))

    def p_task_args_one(self, p):
        'task_args : expression'
        p[0] = (p[1],)
        p.set_lineno(0, p.lineno(1))

    def p_task_args_empty(self, p):
        'task_args : empty'
        p[0] = ()
